...
export const init = (config: Config) => {
    logger.info(`[firestore-glm-discuss] Initialized with config: ${JSON.stringify(config)}`);
}
...
export const receivedAPIResponse = (path: string, historyLength: number, duration: number) => {
...
export const errorCallingGLMAPI = (path: string, error: any) => {
    logger.error(`[firestore-glm-discuss] Error calling GLM API for document '${path}': ${error.message || "UNKNOWN ERROR"}`);
}
