...
import { createIndex, createIndexEndpoint, deployIndex, upsertDatapoint, removeDatapoint, queryIndex, getOperationByName, cancelOperationByName, deleteOperationByName, checkIndexStatus } from "../../src/common/vertex";
...
describe('createIndex', () => {
    ...
    test("should error out if operation from client has error property", async () => {
        mockCreateIndex.mockImplementation(() => Promise.resolve({ error: new Error("test error") }));
        try {
            createIndex();
        } catch (e) {
...
describe('createIndexEndpoint', () => {
    ...
    test("should error out if operation from client has error property", async () => {
        mockCreateIndexEndpoint.mockImplementation(() => Promise.resolve({ error: new Error("test error") }));
...
describe('deployIndex', () => {
    ...
    test("should error out if operation from client has error property", async () => {
        mockDeployIndex.mockImplementationOnce(() => Promise.resolve({ error: new Error("test error") }));
...
describe('upsertDatapoint', () => {
    ...
    test("should post with correct data", async () => {
        await upsertDatapoint("test-index-resource-name", [{ datapoint_id: "test-id", feature_vector: [1, 2, 3] }]);

        const expectedUrl = 'https://us-central1-aiplatform.googleapis.com/v1beta1/test-index-resource-name:upsertDatapoints';
...
describe('removeDatapoint', () => {
    ...
    test("should post with correct data", async () => {
        ...
        const expectedUrl = 'https://us-central1-aiplatform.googleapis.com/v1beta1/test-index-resource-name:removeDatapoints';
...
describe('queryIndex', () => {
    ...
    test("should post with correct data", async () => {
        ...
        const result = await queryIndex(queries, searchResults, endpoint, indexEndpoint);

        const expectedUrl = 'https://test-endpoint.com/v1beta1/projects/dev-extensions-testing/locations/us-central1/indexEndpoints/test-index-endpoint:findNeighbors';
...
describe('deleteOperationByName', () => {
    ...
    test("should delete with correct request", async () => {
        mockDelete.mockImplementationOnce(() => Promise.resolve({ data: "test-data" }));
