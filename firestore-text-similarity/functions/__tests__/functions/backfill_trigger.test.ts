...
jest.mock("firebase-functions", () => {
    return {
        logger: { info: (args: unknown) => mockLoggerInfo(args), error: (args: unknown) => mockLoggerError(args) },
    };
});
...
const getExtensionsMock = () => ({
    runtime: () => ({
        setProcessingState: (state: string, message: string) => mockSetProcessingState(state, message),
    })
});
